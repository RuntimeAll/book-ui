...
/**
 * PRD-C-213 课程计划（脚手架占位页；计划列表 + 课次编辑 + seg 模板由后续页面 agent 接手）。
 * import 一次 api 模块确保类型编译路径通，后续替换为真实调用。
 */
import * as scheduleApi from '@/api/teacher/schedule'

const apiReady = typeof scheduleApi.pagePlans === 'function'
</script>

<template>
  <div class="c213-ph">
    <h2 class="c213-ph-title">课程计划</h2>
    <p class="c213-ph-hint">开发中<span v-show="!apiReady">（api 未就绪）</span></p>
  </div>
</template>
